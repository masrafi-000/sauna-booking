...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    // Init Stripe — light theme colours
    if (SB_Data.stripe_pub_key) {
      state.stripe = Stripe(SB_Data.stripe_pub_key);
      var elements = state.stripe.elements({ locale: "en" });
      ...
      state.cardElement.mount("#sbCardElement");
  ...
  function handlePayment(e) {
    ...
    if (!state.stripe) {
      showFormError(
        "Payment is not configured. Please contact the administrator.",
      );
      return;
    }
    ...
    // Step 1: Create payment intent via WP AJAX
    $.ajax({
      url: SB_Data.ajax_url,
      type: "POST",
      data: {
        action: "sb_create_payment_intent",
        nonce: SB_Data.nonce,
        product_id: state.productId,
      ...
      success: function (res) {
        if (!res.success) {
          showFormError(res.data.message || "Error creating booking.");
          setLoading(false);
          return;
        }
            ...
            if (result.error) {
              ...
              $.ajax({
                ...
                success: function (res2) {
                  ...
                  if (res2.success) {
                    $bookingForm.hide();
                    $("#sbSuccessMessage").text(res2.data.message);
                    $bookingSuccess.show();
                  } else {
                    showFormError(
                      res2.data.message || "Booking error. Please contact us.",
                    );
                  }
                },
                error: function () {
                  setLoading(false);
                  showFormError("Network error confirming booking.");
                },
              });
            }
          });
